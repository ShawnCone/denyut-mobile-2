// OTP related messages
export const CANNOT_SEND_OTP = 'Tidak dapat mengirim OTP'
export const CANNOT_VERIFY_OTP = 'Tidak dapat memverifikasi OTP'
export const OTP_RESENT = 'OTP telah dikirim ulang'

// New posyadu related messages
export const UNABLE_TO_JOIN_POSYANDU = 'Tidak dapat bergabung dengan posyandu'
export const SUCCESSFULLY_JOINED_POSYANDU = 'Berhasil bergabung dengan posyandu'

// Print related messages
export const ERROR_PRINTING_GROWTH_RECORD = 'Gagal mencetak data pertumbuhan'

// Report related messages
export const UNABLE_TO_DOWNLOAD_REPORT = 'Tidak dapat mengunduh laporan'
export const UNABLE_TO_SAVE_REPORT = 'Tidak dapat menyimpan laporan'
export const SUCCESSFULLY_SAVED_REPORT = 'Berhasil menyimpan laporan'
export const GENERATING_REPORT = 'Memuat laporan'
